...
async function discoverLeads(opts = {}) {
  const {
    ...
    perQuery = 20,
    delayMs = 800,
    maxLeads = 0, // stop scanning once this many new leads are found (0 = no cap)
    onProgress = () => {},
    deps = {},
  } = opts;
  ...
  for (const target of targets) {
    ...
    for (const biz of businesses) {
      ...
      // Early-stop: no point scanning hundreds more sites once we have enough.
      if (maxLeads && leads.length >= maxLeads) return { leads, stats };
      if (delayMs) await sleep(delayMs);
    }
  }
