...
// Deliver a prepared list of contacts. Shared by the initial campaign and the
// follow-up sequence. `stage` ('initial' | 'followup1' | 'followup2') picks the
// email copy (via buildEmail) and is stamped on each send-log record so we never
// send the same stage twice. Returns { sent, failed, results }.
async function deliver(toSend, { stage = 'initial', dryRun = true, delayMs = 600, onProgress } = {}) {
  ...
  for (let i = 0; i < toSend.length; i++) {
    ...
    try {
      ...
      if (error) {
        ...
        sent++;
        recorded.push({ email: c.email, segment: c.segment, stage, id: data?.id });
...
// Send a campaign (first-touch cold emails).
//
// opts:
...
async function sendCampaign(opts) {
  ...
  const { sent, failed, results } = await deliver(toSend, {
    stage: 'initial',
    dryRun,
    delayMs,
    onProgress,
...
// Send a follow-up batch. `contacts` should already be the "due" list from
// lib/followup's planFollowup(); we re-check suppression here as a final guard,
// but we intentionally do NOT apply the initial-campaign dedupe (these people
// have of course already had their first email).
//
// opts: { contacts, stage, dryRun, limit, delayMs, onProgress }
// Returns { sent, failed, skipped, results }.
async function sendFollowupCampaign(opts) {
  const {
    contacts = [],
    stage = 'followup1',
    dryRun = true,
  ...
  const { sent, failed, results } = await deliver(toSend, { stage, dryRun, delayMs, onProgress });
  return { sent, failed, skipped, results };
}

module.exports = { planCampaign, sendCampaign, sendFollowupCampaign };
