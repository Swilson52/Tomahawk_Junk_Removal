...
// --- Follow-up copy --------------------------------------------------------
//
// Later touches in the sequence. They're deliberately short — a friendly bump
// (followup1) and a low-pressure final note (followup2) — and reference what we
// do for the recipient's segment without repeating the whole first email.

// One short phrase per segment describing the value we offer them.
const SEGMENT_HOOK = {
  hoa: "keeping your community's common areas clear",
  apartment: 'turning units faster after move-outs',
  realtor: 'getting your listings show-ready',
  property_manager: 'handling cleanouts across your properties',
  contractor: 'hauling off jobsite and demo debris',
  estate: "clearing whatever doesn't sell after a sale",
  office: 'clearing out old office furniture and equipment',
};

function followupParts(c, stage) {
  const b = businessInfo();
  const hook = SEGMENT_HOOK[c.segment] || 'your junk removal';
  if (stage === 'followup1') {
    return {
      subject: c.company
        ? `Following up — junk removal for ${c.company}`
        : 'Following up on junk removal',
      preheader: `A quick note about ${hook}.`,
      lead:
        `${greeting(c)} I reached out a little while back about ${b.name} helping with ${hook}. ` +
        `I know inboxes get buried, so I wanted to float this back to the top. We're licensed, ` +
        `insured, and can usually be out same or next day.`,
      bullets: [],
      cta: `Worth a quick chat about ${orgPhrase(c, 'your properties')}?`,
    };
  }
  // followup2 — final, low-pressure "keep us in your back pocket" note.
  return {
    subject: c.company ? `Last note for ${c.company}` : 'Last note from Tomahawk Junk Removal',
    preheader: "No worries if now isn't the right time.",
    lead:
      `${greeting(c)} I won't keep filling your inbox — this is my last note. If ${hook} ever ` +
      `becomes a headache, keep ${b.name} in your back pocket. We're a quick call away and can ` +
      `usually be out same or next day. If you'd like a standing partner rate, just reply and ` +
      `I'll put one together.`,
    bullets: [],
    cta: "Happy to help whenever the timing's right.",
  };
}
...
// opts.stage selects the sequence step: 'initial' (default) uses the segment's
// first-touch template; 'followup1' / 'followup2' use the shorter follow-up copy.
// Returns { subject, html, text } or throws if the segment is unknown.
function buildEmail(contact, { stage = 'initial' } = {}) {
  const seg = getSegment(contact.segment);
  if (!seg) {
    throw new Error(`Unknown segment "${contact.segment}" for ${contact.email}`);
  }
  const parts = stage && stage !== 'initial'
    ? followupParts({ ...contact, segment: seg.key }, stage)
    : TEMPLATES[seg.key](contact);
  return {
...
module.exports = { buildEmail, TEMPLATES, SEGMENT_HOOK };
